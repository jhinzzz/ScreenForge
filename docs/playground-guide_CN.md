...
**大脑之眼视图**是 Playground 右侧边缘的一个可折叠抽屉，展示 **AI 大脑在每一步实际使用的分层元素树** —— 与压缩器送给模型的过滤集合相同，但以真实的父/子结构重新挂载，而非扁平列表。

**打开方式**：点击窗口右侧边缘的 `TREE` 标签，或在 Playground 任意位置按 **`B`** 键切换抽屉开/关。可以将其固定打开，使其在步骤推进时始终可见。

**读懂树**：
- 以 **ember 橙色**高亮的行是大脑在本步骤操作的元素（当前目标）。在 Web 端，同一 ember 色还会以 **bbox 叠加层**出现在截图上，让你清楚看到大脑精准定位到页面哪个区域。
- **鼠标悬停任意行**（仅 Web）可在截图上看到**蓝色 bbox 叠加层**，方便对任意节点进行空间定位，而不只是当前操作的节点。
- 每个 Web 元素显示其 **`@N` ref 徽章**（与大脑在压缩 DOM 中看到的编号一致）。点击徽章，或在聚焦行上按 **Enter**，可将其**定位器复制到剪贴板** —— 随时可粘贴到 `--action` 命令或测试用例中。

**导航树**：
- 点击任意行的三角形展开/折叠其子树，也可使用键盘：
  - **↑ / ↓** —— 上下移动焦点一行
  - **← / →** —— 折叠/展开焦点节点
  - **Home / End** —— 跳到第一行/最后一行
  - **`/`** —— 聚焦搜索/过滤框；输入文字按 text、role 或 ref 过滤节点；**Escape** 清除
- 树使用**键控协调器**：随着步骤推进，树就地更新，保留你的展开/折叠状态与滚动位置。

**差异徽章**：每步显示 **`+N −N ~N`** 徽章（相较上一步新增/删除/变更的节点数），让你快速感知两次操作之间页面变化了多少。

**移动端诚实降级**：Android 和 iOS **没有 `@N` ref 徽章，也没有 bbox 叠加层** —— 这些在移动端采集中确实不存在，并非隐藏。悬停轨道变为**琥珀色**作为提示，并有一次性可关闭的说明通知。在 **Android** 上你会得到一棵**真正分层的树**（来自 `dump_hierarchy` 的真实父/子嵌套），相对于 LLM 收到的扁平元素列表是真实的改进。在 **iOS 上 DOM 树暂不支持**：旁路采集复用 Android XML 谓词，但 WDA `source()` 返回的是属性不同的 XCUITest XML，因此没有节点能通过过滤 —— 树指示灯保持熄灭、不显示任何树（iOS XCUITest/WDA 谓词是后续待加项）。
...
- **大脑之眼视图只读且仅限单会话**：DOM 树面板仅支持展开/折叠、搜索、复制定位器操作，不支持编辑或对元素执行动作。反向空间查找（点击截图像素 → 高亮对应树节点）与 SSE 差异流推送留作未来迭代。
