...
**移动端诚实降级**：Android 和 iOS **没有 `@N` ref 徽章，也没有 bbox 叠加层** —— 这些在移动端采集中确实不存在，并非隐藏。悬停轨道变为**琥珀色**作为提示，并有一次性可关闭的说明通知。你得到的是**真正分层的树**（来自 `dump_hierarchy` / WDA `source` 的真实父/子嵌套），这相对于 LLM 收到的扁平元素列表是真实的改进。
