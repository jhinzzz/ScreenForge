...
- **`inspect_ui` 案例记忆提示**：传入可选的 `task` 标签，返回体的 `memory` 字段会携带该任务上次成功的信息 —— Agent 在**行动前**看到的提示，结合实时 UI 树自行判断，绝不会被自动重放。标签需与上一次运行记录的一致（`--action-name`，否则为 `"{action}:{locator_value}"`）；用 `load_case_memory` 可查看已记录的标签。详见 [Agent 指南](docs/agent_guide.md)。
