...
### 修复
- **浅色主题下，空闲的 live view 看起来像是面板没加载出来。** 三处问题，深色下全都看不出来，
  因为它们是两套主题之间**幅度**的差异，而不是颜色的差异。`--bg-0`（空闲面板自身的表面）
  比它所在的面板低 1.206 个对比度台阶，而深色是 1.062——所以挨着近白的代码面板，它读起来
  就是一块灰盒子，而深色下这一对只差 5 级、读起来是同一个表面。镜片在纸面上是**故意反向**的
  （玻璃在纸上是深色物体），但同样深了 1.2 倍，所以表面修正之后它不再像玻璃，变成一块灰盘。
  另外对焦弧线是经 `--ember-soft` 过渡的，而这支色在深色下比 ember 亮（#ffb38a）、在浅色下
  比 ember 暗（#9a3412），于是弧尾在深色下朝背景淡出、在浅色下朝反方向加深——实测浅色弧线
  对比度最强的地方是它的**末端**，看着像圆环上的一道划痕而不是拖尾的光。现在弧线在单一色道上
  按 alpha 递变，"朝身后的背景淡出"成了构造上的必然，两个表面台阶也都按深色的深度重新标定，
  同时保留各自本来的方向。
- **取景器的四个角标在浅色下低于图形对比度下限。** 实测 2.78:1，而非文本图形需要 3:1
  （深色是 7.52:1）。两个原因，且只修第一个不够：`opacity: .9` 会朝身后的颜色稀释，在深色下
  是朝黑（无损），在浅色下是朝白——去掉它只从 2.74 升到 2.78，底下那个表面才是更大的一半。
  两处一起修：浅色 3.11:1，深色 7.18:1，而且是从渲染出的像素上量的，不是从 token 值算的。
- **窗口宽度小于顶栏固有宽度时，playground 会横向滚动。** 顶栏是 `flex-wrap: nowrap`，
  只在 720px 以下才允许换行，于是从 721px 一直到顶栏自身的固有宽度之间，整个文档都在
  横向滚动。而这个固有宽度取决于内容——空闲时约 1019px，一旦填入 run id、生成的文件名
  和探测到的编辑器就涨到约 1330px——所以 1366px 的笔记本空闲时正常、跑起来就横滚，
  换任何一个固定断点都只能修好一种状态。现在顶栏按固有宽度换行而不是按魔法数字，配合
  `min-height`（固定 `height` 会裁掉第二行）和仅在挤压时才生效的省略号，因此只要还有
  空间，文件名依然完整显示。已验证 27 个宽度（1920→320）× 两套主题 × 空闲/填充态下
  横向溢出全部为 0。
- **Brain's Eye View 抽屉在代码认为它已关闭时仍然可见。** 固定它、关闭它、刷新——固定态
  的 CSS 会覆盖关闭态的 transform，于是面板完整地回到屏幕上，而 `isOpen === false`。
  `showTreeForStep()` 开头就是 `if (!isOpen) return`，所以这个可见的面板永远不再更新：
  步骤不断推送，它却一直停在 "Waiting for the first step"；在 1360px 以下它还会永久
  盖住代码面板。现在固定与打开是同一个状态——固定即打开，关闭即取消固定——"关闭"才真正
  意味着在屏幕之外。
- **已关闭的抽屉仍留在键盘 Tab 序列里，且 Escape 关不掉它。** Tab 能走到一个停在
  `translateX(100%)` 屏幕外的面板的固定、关闭、搜索控件上；打开抽屉时焦点不会移入；
  而 Escape 只绑定在搜索框（用于清空文本）和引导浮层上。现在抽屉关闭时为 `inert`，
  打开时接收焦点、关闭时把焦点交还给切换标签，Escape 会关闭它——如果搜索框里有文本则
  先清空，一次按键绝不做两件事。它保持非模态（`role="complementary"`，无 `aria-modal`），
  所以 Tab 仍会有意地跳到后面的页面上。恢复持久化的打开状态时刻意不抢焦点：用户并没
  有主动要求打开它。
- **六个强调色里有五个在被用作底色时无视浅色主题。** 此前只有 ember 做了 token 化，
  其余强调色在 38 条全局规则里被写成深色主题的裸 `rgba()` 通道值，于是在纸感主题下，
  状态胶囊的*文字*跟随主题，而它自己的边框和背景仍是深色——"live" 胶囊是
  `#047857` 的文字配 `rgb(110,231,183)` 的边框。现在每个强调色都有按主题定义的通道
  token（`--live-rgb`、`--blue-rgb`……），代入各条规则原有的 alpha，且 alpha 全部
  逐字保留。已通过对比两套主题下每一条计算后声明来验证：深色主题除一处有意的合并
  （空闲胶囊的浅琥珀现在与它自己的标签同色）外完全一致，浅色主题的变化全部是预期中的
  换色，无 alpha 漂移，也没有任何一处静默变透明。
- **浅色主题下有三个强调色在底色跟随主题后跌破 AA。** 每个胶囊都用它自己书写的强调色
  给自身背景上色，所以跟随主题后的底色比纯纸面更贴近文字：`--blue`、`--amber`、
  `--live` 在自己的胶囊上实测 4.23 / 4.12 / 4.48:1，而在纯面板上是达标的 4.5:1 以上。
  现将三者分别加深 8% / 7% / 2%，浏览器实测 4.72 / 4.55 / 4.52:1。在浅色表面上加深
  只会提升对比度，所以标题、链接和动作类型也一并变好。
- **实时视图的画面角标在浅色主题下几乎看不见（1.63:1）。** 它的背景在两套主题下都是
  钉死的暗色墨块——它压在被测应用的截图上，那始终是暗的——但标签用了跟随主题的强调色，
  于是在纸感主题下变成深绿压近黑。现在改用与主题无关的强调色，也就是 `--on-shot` 早已
  为这层图像上的文字定下的规则：5.85:1 与 5.34:1。
- **IDE 按钮的图标在浅色主题下仍是深蓝。** 它的 SVG 把深色主题的两个 hex 写死了，
  所以图标不跟随它外面的按钮。现在改用既有 token（`var()` 在 SVG 呈现属性里是有效的）。
- **Prism.js CDN 不可达时，playground 实时视图不再整页空白。** Prism 仅从 CDN 加载，
  而代码面板无守卫地调用了 `Prism.highlight()`。在离线 / 内网隔离 / 代理拦截的环境下，
  它会从 SSE 步骤处理器里抛出 `ReferenceError`——于是不只是语法高亮失效，截图、操作
  历史、胶片条也一并停止渲染，页面永久卡在 "Connecting"。现在高亮会降级为转义后的
  纯文本，且图像先于代码面板渲染，单个面板的失败无法再拖垮其他面板。
- **回放报告现在会在 `click` 与 `assert` 步骤上画出目标定位框。** 舞台叠框此前只能
  解析 `#id` 形态（外加一个生成器从不产出的 `text=` 死分支），而录制器实际写入的是
  `get_by_text(...)`、`get_by_role(..., name=...)`、`get_by_label(...)`、
  `get_by_placeholder(...)` 和 `[aria-label='...']`。结果是：`fill` 步骤有框，而
  查看报告时最需要检视的 click / assert 步骤反而没有。现在这些形态均可解析；
  不支持的 CSS 选择器仍优雅降级为不画框。
- **移除了在高分屏上会画错定位框的回退分支。** 对于缺少 `vw`/`vh` 的旧 `review.json`，
  定位框会回退到 `naturalWidth/dpr` 估算，而 `dpr` 缺省为 1，在 2× 屏上约按半尺寸绘制
  （且在换图瞬间读到的是上一帧的尺寸）。现在只有在录制的 CSS 视口存在时才绘制——
  不画框比画错位置更诚实。
- **playground 空闲代码面板的四行占位注释挤成了一整段。** 它们是仅由源码换行分隔的
  行内 `<span>`，而 `div` 会把换行折叠成空格——于是 `#` 被埋进句子中间。该区块现在使用
  `white-space: pre-wrap`，既保留换行又仍能自动折行。
- **窗口较矮时空闲实时视图会溢出，裁掉 `--playground` 提示。** 面板主体是无滚动的居中
  flex 容器，而 `align-items: center` 不会施加收缩压力，占位块因此始终保持自然高度，
  底部提示被挤到面板之外。现在它受面板高度约束，并按优先级逐级舍弃氛围元素：主体高度
  低于约 250px 时取景器整体退场（而不是缩成一道橙色碎线），低于约 200px 时 STANDING BY
  角标与首段说明也一并隐去——标题和该执行的命令始终保留。该面板刻意不做成滚动容器：
  空闲面板出现滚动条，比少显示一些内容更糟。
- **空闲面板的入场动画忽略了 `prefers-reduced-motion`。** 交错的 `:nth-child` 规则
  优先级高于 reduce 覆盖规则，明确要求减少动效的读者仍会看到文字动画。
- **浅色模式下的空闲实时视图看起来像另一个部件。** 它的强调色角标把深色主题的熔金橙写成了
  内联 `rgba()` 字面量，于是在浅色底上，角标的底色与它自己的文字是两种橙；对焦弧与测光扫描线
  都经由 40% 透明度的光晕插值，而这在近白背景上根本不可见——浅色主题画出的弧长只有深色主题的
  约十分之一；镜身则复用了一个在两个主题里都**比面板更亮**的表面色，浅色下因此成了 1.21:1 的
  白色圆盘，把自己的对焦环吞掉了。现在熔金橙的底色、描边与镜身玻璃都是按主题定义的 token——
  玻璃在浅色下比面板更暗、在深色下比面板更亮，刻意反转，使它在两侧都读作有深度的实体——描边
  改为经由不透明色插值，柔和度交给 `opacity` 而非 alpha 承担。已在冻结的同一动画相位下验证：
  两个主题画出同长度的弧。
- **浅色主题下，落在自身角标上的熔金橙文字对比度低于 AA。** 这个焦橙是按纯净纸面背景挑的
  （5.3:1），但每个熔金橙角标都会用 ember 底色给自己的背景上色，叠上之后 `STANDING BY` 标签、
  `--playground` 代码片、时间轴 seed、已固定节点按钮全部落到 3.7–4.4:1——低于这种小字号所需的
  4.5:1 下限。浅色 `--ember-soft` 现在改为更深的焦橙，且是按每个角标真实合成出来的背景（而不是
  它后面的面板）测量的：全部落在 5.2–6.5:1，深色主题保持 10–11:1 不变。
- **playground 的熔金橙强调色在约 25 条规则里被写成裸 `rgba()` 三元组。** 每一处都硬编码了
  **深色**主题的 ember，于是除了少数已 token 化的地方，所有 ember 着色的行背景、描边、渐变与
  光晕在浅色主题下依旧是深色主题那支橙。现在颜色通道收为每主题一个 token（`--ember-rgb`），
  代入各规则自己的 alpha——这些 per-rule alpha 是有意为之（0.05 的行底色与 0.45 的描边是两件
  事），全部原样保留，消失的只有被复制的色相。验证方式是对页面上每一条计算后的颜色声明做 diff：
  深色主题在 20,681 条声明上逐字节相同，浅色主题的每一处变化都是预期的换色且 alpha 无漂移。
  回放报告的熔金橙强调色做了同样处理（它只有深色，所以那边纯属去重——6,479 条计算值未变）；
  它的 token 也从 `--signal*` 改名为 `--ember*`，与 playground 对齐——同样四支颜色套两套命名，
  两个文件根本没法对着看。已按"纯改名"验证：渲染出的报告在 1,090 条声明上计算值零差异。

### 变更
- **playground 的空闲实时视图从加载转圈改为取景器。** 旧的旋转光环把圆形渐变遮罩
  套在圆角**方形**上，光晕因此糊在四角而非沿边缘扫过。现在换成四个取景角标、一道
  测光扫描线和一枚对焦镜头——与界面其余部分共用同一套准星 / 胶片语言。
- **回放报告的强调色改为熔金橙 `#ff7849`，与 playground 一致。** 两个界面此前使用两个
  几乎相同的暖橙（`#FFB84D` 与 `#ff7849`），读起来像漂移而非设计意图。报告保留自己的
  显示字体与回放概念（录制红点、均衡器、胶片颗粒）——仅共享品牌强调色。附带效果：
  强调色不再与同为 `#FFB84D` 的 `fill` 动作分类色撞色。

## [0.11.0] - 2026-07-28

### 变更
- **LLM 决策调用在端点支持时会请求 JSON 模式。** 大脑的动作/计划/推理调用会传入
  `response_format={"type":"json_object"}`，减少格式错误的响应（以及由此产生的重试）。
  按端点探测一次：不支持该参数的 OpenAI 兼容端点会被记住并不再发送，从而保持“任意兼容
  端点”的承诺——这类端点只付出一次失败调用，而非每次请求都失败。纯视觉调用（自愈、
  视觉回退）保持不变。
- **`--goal` 循环在连续 `not_found` 后会停止，而不再耗尽整个步数预算。** `not_found`
  不计入失败熔断（以便给视觉回退留机会），但模型若一直返回它就毫无进展，却每步消耗
  一次 LLM 调用，直到 `--max-steps`。现在连续 `not_found` 的次数上限为 `--max-retries`，
  达到后以 `E025` 停止。

### 新增
- **自动启动目标现在可通过环境变量配置。** `APP_ENV_CONFIG` 此前是一个硬编码且全空的
  Python 字典，pip 安装的用户不编辑 site-packages 就无法设置要自动启动的应用。现在它
  由 `APP_TARGET_<ENV>_<PLATFORM>` 环境变量构建（如 `APP_TARGET_DEV_ANDROID`）；
  留空（默认）仍表示“不自动启动”。

### 修复
- **`--session-end` 在结束视频录制时不再可能永久卡死。** 停止会话时会向 `simctl`
  录制进程发送信号，然后调用裸的 `os.waitpid(pid, 0)`——若该进程忽略信号，它会无限
  阻塞。现在改为等待有限的宽限期，超时后升级为 `SIGKILL`，从而保证结束会话总能返回。
- **非中文页面不再在 L1 缓存里哈希碰撞。** 页面指纹此前只保留汉字，于是任何纯
  英文页面都得到空指纹、同一个常量 hash —— L1 精确键随后会重放一条记录在**另一个**
  英文页面上的动作（重放了错的动作）。现在没有中文锚点的页面会回退到拉丁文本锚点，
  并保持同样的动态数据免疫（抹除数字/符号，所以 "Balance 100" 与 "Balance 9999"
  仍然匹配）。有可用中文锚点的页面行为不变。
- **L2 语义缓存现在会先校验 `css` 定位器再重放。** `css` 是 Web 端优先级最高的
  定位器，但重放前的安全校验此前对所有 `css` 决策一律放行、不做校验。由于 L2 是
  **跨页面的全局**缓存，一条在某页面学到的 `#username` 决策可能被重放到另一个根本
  没有该元素的页面。现在校验会确认压缩树能还原的两种 `css` 形态（`#id` 与
  `[name="x"]`）；无法解析的选择器（class/标签/后代）以及树中没有该属性的情况仍然
  放行，绝不误伤任何有效缓存命中。仅匹配**简单**选择器：复合选择器
  （`#login-form input`）或转义后的 id（`#user\.email`，由含 `.`/`:` 的 id 生成）
  不可能等于原始属性值，因此一律放行，而不是把有效命中丢掉。
- **过期缓存条目现在会在写入时被清理，而不再被反复重写回磁盘。** TTL 清理此前只
  在读取路径执行，且仅在缓存**命中**时才把结果存盘；而写入路径重新加载并整体保存
  整个文件却从不清理——于是过期条目被永久重写回磁盘，缓存无限膨胀。现在写入会在
  保存前先清理（无额外 I/O——写入本就要加载并保存）。
- **`CACHE_MAX_SIZE_MB` 现在真正生效。** 该配置此前从环境变量读取并传入缓存，却从未
  被使用，因此缓存没有体积上限（只有 TTL 约束）。现在写入会淘汰最久未访问的条目，
  直到缓存体积符合上限；刚写入的条目永不被淘汰。
