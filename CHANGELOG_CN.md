...
- **空闲实时视图会裁掉那行告诉你如何开始的提示。** 窗口较矮时它的内容溢出面板，
  `--playground` 命令被挤到面板之外。现在面板会随着变矮逐级舍弃氛围元素——先是取景器，
  再是待机角标——标题和该执行的命令始终保留。
- **Brain's Eye View 抽屉可能停在屏幕上显示过期内容。** 固定它、关闭它、刷新：它会完整
  地回到屏幕上，却一直停在 "Waiting for the first step"，而步骤仍在不断推送；窗口较窄时
  它还会盖住代码面板。现在固定与打开是同一个状态，"关闭"就真的是关闭。
- **抽屉无法用键盘操作。** Tab 会走进一个已关闭、在屏幕之外的面板的控件；打开时焦点不会
  移入；Escape 关不掉它。现在 Tab 会跳过已关闭的抽屉，打开时焦点移入、关闭时交还，
  Escape 会关闭它——搜索框里有文本时先清空，一次按键绝不做两件事。Tab 仍然可以离开抽屉
  回到后面的页面：它是侧边栏，不是模态框。
- **强调色无视浅色主题。** 状态胶囊、操作行、差异角标之类的元件，文字用的是浅色主题的颜色，
  边框和背景却仍是深色主题的——"live" 胶囊是深绿的文字配薄荷绿的边框。"在编辑器中打开"
  的图标也是同样的原因一直是深蓝。现在它们都跟随主题。
- **浅色主题下有几处淡到看不清。** 落在自身底色上的强调色文字、截图上的实时/空闲角标，
  以及取景器的四个角标，都低于 WCAG AA 对比度下限——其中角标几乎完全看不见。现在全部达标，
  而且是按每一处真实合成出来的背景测量的，不是按它后面的面板。
- **空闲实时视图在两套主题下读起来不像同一个部件。** 在浅色下，面板像一块没加载出来的
  盒子挨着代码面板，镜头是一块灰盘而不是玻璃，对焦弧看着像圆环上的一道划痕而不是拖尾的光。
  现在两套主题在深度和受光方式上一致，同时在纸面确实需要反向的地方保留反向。
- **语法高亮的 CDN 不可达时，实时视图不再整页空白。** 在离线、内网隔离或被代理拦截的环境下，
  这一个加载失败会把截图、操作历史和胶片条一起拖垮，页面永久卡在 "Connecting"。现在高亮
  降级为纯文本，其余部分照常工作。
- **空闲代码面板的四行占位注释挤成了一整段。**
- **空闲面板的入场动画忽略了 `prefers-reduced-motion`。**
- **回放报告现在会在 `click` 与 `assert` 步骤上画出目标定位框。** 它此前只能按 `id` 定位
  元素，于是查看报告时最需要检视的那些步骤，恰恰是没有框的。
- **回放报告不再在高分屏上画错定位框。** 对旧版本录制的报告，它只能猜测屏幕缩放比例，
  在 2× 屏上约按半尺寸绘制。现在只有知道框该画在哪里时才画——不画框比画错位置更诚实。

### 变更
- **playground 的空闲实时视图是取景器，而不是加载转圈。** 四个取景角标、一道测光扫描线
  和一枚对焦镜头——与界面其余部分共用同一套准星 / 胶片语言，取代了光晕糊在四角的旋转光环。
- **回放报告的强调色改为与 playground 相同的熔金橙。** 两个界面此前使用两个几乎相同的
  暖橙，读起来像漂移而非设计意图。报告保留自己的显示字体与回放概念——录制红点、均衡器、
  胶片颗粒——仅共享品牌强调色。它也不再与 `fill` 动作使用的颜色撞色。
